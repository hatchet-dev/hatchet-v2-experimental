...
func (t *V2WorkflowRunsService) V2WorkflowRunGet(ctx echo.Context, request gen.V2WorkflowRunGetRequestObject) (gen.V2WorkflowRunGetResponseObject, error) {
	...
	workflowRunPtr, err := t.config.OLAPRepository.ReadWorkflowRun(requestContext, sqlchelpers.UUIDFromStr(tenant.ID), sqlchelpers.UUIDFromStr(workflowRunId.String()))
	...
	result, err := transformers.ToWorkflowRunDetails(taskRunEvents, &workflowRun, shape)
